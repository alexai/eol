 ...
 * @copyright 2009
 */
$qr1 = mysql_query("SELECT * FROM `gb_students_tb` WHERE `stu_id` = '$_SESSION[uid]';");
?>  
  <!-- calendar stylesheet -->
...
lhgchk.rules = [
	...
	{ name : 'paremail', required : false, type : 'email', warn : '请输入正确的Emial格式' },
	{ name : 'tel', required : false, type : 'phone', warn : '请输入正确的电话格式' }
];
window.onload = function()
{
    lhgchk.regform( { form : 'rs', mode : 0 } );
}
...
<body>
<form method="post" id="rs" action="profile.php">
		<fieldset>
			...
			<dl>
				...
				<dd>
				<input type="text" name="birthday" id="birthday" value=<?php echo mysql_result($qr1,0,birthday); ?> /><button type="reset" id="f_trigger_b">选择</button>
				<script type="text/javascript">
				    Calendar.setup({
			...
			<dl>
				...
				<dd><input id="email" name="email" type="text" value=<?php echo mysql_result($qr1,0,email); ?>></dd>
			</dl>
			<dl>
				<dt><label for="addr">住址：</label></dt>
				<dd><input id="addr" name="addr" type="text" value=<?php echo mysql_result($qr1,0,addr); ?>></dd>
			</dl>
			<dl>
				<dt><label for="paremail">家长信箱：</label></dt>
				<dd><input id="paremail" name="paremail" type="text" value=<?php echo mysql_result($qr1,0,par_email); ?>></dd>
			</dl>
			<dl>
				<dt><label for="tel">联系电话：</label></dt>
				<dd><input id="tel" name="tel" type="text" value=<?php echo mysql_result($qr1,0,tel); ?>></dd>
			</dl>
			<dl>
