...
<body>
<h2>选择简答题</h2>
<form name="select_item" mothod="post" action="add_paper_4.php">
<ol>
<?php 
if (!isset($_REQUEST["hooker"])) {
	...
	$IDGROUP  = "";
	foreach ($_REQUEST["essaygroup"] as $v) {
		$IDGROUP = $IDGROUP.$v.",";
	}
...
if ($VAR == 0) {
	$TOTAL = mysql_num_rows(mysql_query("SELECT `item_id` FROM `gb_items_warehouse` WHERE `item_type` = '4';"));
	$PAGES = ceil($TOTAL/$PAGE_SIZE);
	$offset = $PAGE_SIZE*($page - 1);
	$rs = mysql_query("SELECT * from `gb_items_warehouse` WHERE `item_type` = '4' ORDER BY `item_id` DESC LIMIT $offset,$PAGE_SIZE;");
} else {
	$TOTAL = mysql_num_rows(mysql_query("SELECT `item_id` FROM `gb_items_warehouse` WHERE `item_type` = '4' AND `item_category` = '$VAR';"));
	$PAGES = ceil($TOTAL/$PAGE_SIZE);
	$offset = $PAGE_SIZE*($page - 1);
	$rs = mysql_query("SELECT * from `gb_items_warehouse` WHERE `item_type` = '4' AND `item_category` = '$VAR' ORDER BY `item_id` DESC LIMIT $offset,$PAGE_SIZE;");
}

//count display rows.
while ($myrow = mysql_fetch_array($rs)) {
	$stem = mysql_query("SELECT `item_type4_stem` FROM `gb_item_type4` WHERE `item_id` = '$myrow[0]';");
...
//display
echo "<li><input type=checkbox name=essaygroup[] value=".$myrow[item_id]." />";
...
echo "<span class=s14>".mysql_result($stem,0,0)."</span>";
...
if ($page > 1) {
	echo "<a href='add_paper_4.php?page=".$first."'>首页</a>&nbsp;&nbsp;";
	echo "<a href='add_paper_4.php?page=".$prev."'>上一页</a>&nbsp;&nbsp;";
} 

if ($page < $PAGES) {
	echo "<a href='add_paper_4.php?page=".$next."'>下一页</a>&nbsp;&nbsp;";
	echo "<a href='add_paper_4.php?page=".$last."'>末页</a>";
}
echo "</div>";
?>
<input type="button" value="生成试卷" onclick="javascript:self.location='admin_view_paper.php'" />
